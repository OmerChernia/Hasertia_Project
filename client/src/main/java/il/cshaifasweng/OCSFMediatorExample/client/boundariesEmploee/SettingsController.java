...
public class SettingsController implements Initializable {
    ...
    private void updateCredentials() {
        ...
        if (name.isEmpty() && user.isEmpty() && password.isEmpty() && confirmPassword.isEmpty() && bio.isEmpty()) {
            ...
            NotificationsBuilder.create(NotificationType.ERROR, ConstantsPath.MESSAGE_INSUFFICIENT_DATA);
            return;
        }
        ...
        if (user.length() < 4) {
            txtUser.requestFocus();
            Animations.shake(txtUser);
            NotificationsBuilder.create(NotificationType.ERROR, ConstantsPath.MESSAGE_ENTER_AT_LEAST_4_CHARACTERES);
            return;
        }
        ...
        if (!confirmPassword.equals(password)) {
            ...
            Animations.shake(txtPassword);
            Animations.shake(txtConfirmPassword);
            NotificationsBuilder.create(NotificationType.ERROR, ConstantsPath.MESSAGE_PASSWORDS_NOT_MATCH);
            return;
        }
    ...
    private void showFileChooser() {
        ...
        if (imageFile != null) {
            if (imageFile.length() < LIMIT) {
                ...
                AlertsBuilder.create(AlertType.SUCCESS, stckSettings, rootSettings, rootSettings, MESSAGE_PROFILE_IMAGE_SAVED);
                loadProfileImage();
            } else {
    ...
    private void loadProfileImage() {
        // Load dummy image
        InputStream imageStream = getClass().getResourceAsStream("/media/icon.jpeg");
        if (imageStream == null) {
            Logger.getLogger(SettingsController.class.getName()).log(Level.SEVERE, "Image not found: /media/icon.jpeg");
        } else {
